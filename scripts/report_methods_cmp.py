...
"""读 methods_cmp.csv → 仿 full_compare_K=5.txt 格式的报告:
  ...
  2) 按类别聚合(均值 + 比值均值)
  3) Auto / m3 输给 Ocean 的 case(ratio>1,降序,赢/输计数 + 几何均值)
...
def main():
    ...
    out = []
    out.append("[compute-only:稀疏法=各阶段求和去 h2d/d2h(对标 full_compare);Ocean=GPU 阶段求和;HSMU=time2]  ms")
    out.append("class name                        n    cuSP   Ocean    HSMU  serial     m3    Auto Auto/Oce  m3/cu  m3/Oce")
    out.append("-" * 116)
    for r in rows:
        cu, oc, hs, se, m3, au = (fnum(r, k) for k in ("cu", "Ocean", "HSMU", "serial", "merge3", "Auto"))
        n_str = f"{r['_n']:,}"
        def ratio(a, b): return a / b if (a and b and b > 0) else None
        out.append(f"{CLASS_TAG.get(r['_class'],'?'):>5} {r['matrix']:<24}{n_str:>7}"
                   f"{fmt(cu)}{fmt(oc)}{fmt(hs)}{fmt(se)}{fmt(m3)}{fmt(au)}"
                   f"{fmt(ratio(au, oc), 7, 2)}{fmt(ratio(m3, cu), 7, 2)}{fmt(ratio(m3, oc), 7, 2)}")
    ...
    out.append("-" * 116)
    out.append(f"{'class':<20}{'#':>4}{'Ocean':>9}{'HSMU':>9}{'cuSPARSE':>10}{'serial':>9}{'m3':>8}{'Auto':>8}{'Auto/Oce':>10}{'m3/cu':>8}{'m3/Oce':>9}")
    for c in CLASS_ORDER:
        sub = [r for r in rows if r["_class"] == c]
        if not sub: continue
        oc = mean([fnum(r, "Ocean") for r in sub]); hs = mean([fnum(r, "HSMU") for r in sub])
        cu = mean([fnum(r, "cu") for r in sub]); se = mean([fnum(r, "serial") for r in sub])
        ...
        rmc = mean([fnum(r, "merge3") / fnum(r, "cu") for r in sub
                    if fnum(r, "merge3") and fnum(r, "cu")])
        rmo = mean([fnum(r, "merge3") / fnum(r, "Ocean") for r in sub
                    if fnum(r, "merge3") and fnum(r, "Ocean")])
        out.append(f"{c:<20}{len(sub):>4}{fmt(oc,9,2)}{fmt(hs,9,2)}{fmt(cu,10,2)}{fmt(se,9,2)}"
                   f"{fmt(m3,8,2)}{fmt(au,8,2)}{fmt(ra,10,2)}{fmt(rmc,8,2)}{fmt(rmo,9,2)}")

    # ---- 输给 Ocean 的 case ----
    def lost_section(label, col):
        pairs = []
        for r in rows:
            t = fnum(r, col); oc = fnum(r, "Ocean")
            if t and oc and oc > 0:
                pairs.append((r, t / oc))
        win = [p for p in pairs if p[1] <= 1.0]
        lose = [p for p in pairs if p[1] > 1.0]
        lose.sort(key=lambda p: -p[1])
        out.append("")
        out.append("=" * 100)
        out.append(f"{label} vs Ocean:赢 {len(win)} / 输 {len(lose)},几何均值 {gmean([p[1] for p in pairs]):.3f}×")
        out.append("-" * 100)
        out.append(f"{'name':<16}{'n':>8}{'C_nnz':>10}{'Ocean':>10}{label:>14}{'ratio':>9}")
        for r, ratio in lose[:20]:
            cnnz = r.get("cnnz", "")
            out.append(f"{r['matrix']:<16}{r['_n']:>8}{cnnz:>10}{fmt(fnum(r,'Ocean'),10,3)}{fmt(fnum(r,col),14,3)}{ratio:>8.2f}×")
        if len(lose) > 20:
            out.append(f"  ... 另有 {len(lose)-20} 个")

    lost_section("Auto", "Auto")
    lost_section("m3", "merge3")
